...
const Select = ({ label, options, ...props }) => (
  <div className="mb-4">
    <label className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
    <select 
      className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white"
      {...props}
    >
...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    if (user === "administrador" && pass === "grapaz2026") {
      onLogin();
    } else {
    ...
    <div className="min-h-screen bg-blue-900 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-blue-800">Gestão Grapaz</h1>
...
const Dashboard = ({ bids }) => {
  const stats = {
    ...
    receivable: bids
      .filter(b => ['won', 'partial', 'delivered'].includes(b.status))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-l-4 border-blue-500">
          <p className="text-gray-500 text-sm">Total de Pregões</p>
          <p className="text-3xl font-bold text-blue-800">{stats.total}</p>
        </Card>
        <Card className="border-l-4 border-green-500">
          <p className="text-gray-500 text-sm">Processos Vencidos</p>
          <p className="text-3xl font-bold text-green-700">{stats.won}</p>
        </Card>
        <Card className="border-l-4 border-red-500">
          <p className="text-gray-500 text-sm">Processos Perdidos</p>
          <p className="text-3xl font-bold text-red-700">{stats.lost}</p>
        </Card>
        <Card className="border-l-4 border-yellow-500">
          <p className="text-gray-500 text-sm">Valor em Aberto (R$)</p>
          <p className="text-3xl font-bold text-yellow-700">
            {stats.receivable.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
          </p>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        ...
        <Card>
          <h3 className="font-bold text-lg mb-4 text-blue-900">Avisos Recentes</h3>
          <div className="bg-yellow-50 p-4 rounded-md border border-yellow-200">
            <p className="text-yellow-800 text-sm">Verifique os prazos de entrega dos pregões vencidos na aba "Vencidos".</p>
          </div>
        </Card>
      </div>
    </div>
  ...
  return (
    <Card className="max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Select 
          label="Modalidade" 
          options={['Pregão Eletrônico', 'Pregão Presencial', 'Dispensa Eletrônica', 'Chamamento Público', 'Concorrência']}
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="bg-gray-200 p-1 rounded-lg flex">
          <button 
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium ${!viewPast ? 'bg-white shadow text-blue-800' : 'text-gray-600'}`}
          >
            Próximos
          </button>
          <button 
  ...
  return (
    <Card className="border border-blue-100">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h4 className="font-bold text-lg">{localBid.orgao}</h4>
    ...
    <div className="space-y-8">
      ...
      {Object.keys(groupedByCity).map(city => (
        <div key={city} className="space-y-4">
          ...
          {groupedByCity[city].map(bid => (
            <WonBidCard key={bid.id} bid={bid} onSave={onSaveBid} />
          ))}
        </div>
      ))}
    </div>
...
const Payments = ({ bids, onUpdateBid }) => {
  const deliveredBids = bids.filter(b => ['delivered', 'paid'].includes(b.status));
  const totalReceivable = deliveredBids
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end border-b pb-4">
        <h2 className="text-2xl font-bold text-blue-900">Pagamentos e Recebíveis</h2>
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h4 className="font-bold text-lg text-gray-800">{bid.orgao}</h4>
                {bid.status === 'paid' && <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full font-bold">PAGO</span>}
              </div>
              <p className="text-gray-600 text-sm">Cidade: {bid.cidade} | Pregão: {bid.numeroPregao}</p>
              <p className="mt-2 font-mono font-medium text-blue-900">
